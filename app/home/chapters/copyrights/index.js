...
const CopyrightsScreen = () => {
  ...
  useEffect(() => {
    ...
    const fetchUserLevels = async () => {
      ...
      try {
        const response = await fetch(`${API_URL}/api/user-levels?email=${userInfo.email}&chapter=copyrights`, {
          method: "GET",
          headers: {
  ...
  const handleLevelPress = (level) => {
    router.push(`/home/chapters/copyrights/level${level}`);
  };

  return (
    <ImageBackground
      source={require("../../../../assets/images/copyrightbg.jpg")}
      style={[styles.background, { width, height }]}
      resizeMode="cover"
    >
      <View style={styles.overlay}>
        <StatusBar hidden={true} />
        <Text style={styles.title}>Copyright Levels</Text>
